...
def parse_multi_sheet_txt(raw_text):
    ...
    for line in raw_text.splitlines():
        ...
        if upper_line.startswith("SHEET:"):
            ...
            # Format: @CHARGER: ROW, [01 TO 04], Label
            match = re.match(r'@(CHARGER|CHOKE):\s*([A-Z0-9]+),\s*\[(\d+)\s*TO\s*(\d+)\](?:,\s*(.*))?', line, re.I)
            if match:
                current_symbols.append({
                    "type": match.group(1).upper(), "row": match.group(2).upper(),
            ...
            if len(parts) >= 2:
                ...
                for match in matches:
                    ...
                    for i in range(start, end + 1):
                        current_rows.append({"Row ID": rid, "Function": func_text, "Terminal Number": str(i).zfill(2), "Num": i})
...
def draw_page_template(c, width, height, footer_values, sheet_num, page_heading):
    ...
    for i in range(8):
        ...
        c.drawCentredString(x_c, PAGE_MARGIN + (30 if i >= 4 else 10), val.upper())
    return info_x
        ...
        if not df.empty:
            ...
            for rid, group in df.groupby('Row ID', sort=False):
                ...
                for idx, t in enumerate(group.to_dict('records')):
                    ...
                    c.setFont("Helvetica-Bold", 8)
                    c.drawCentredString(tx, y_curr+17, t['Terminal Number'])
        ...
        for sym in sheet.get('symbols', []):
            if sym['type'] in symbols_map and sym['row'] in row_y_map:
                ...
                if not row_data.empty:
                    center_x = (row_data['x_coord'].min() + row_data['x_coord'].max()) / 2
                    target_y = row_y_map[sym['row']] + 60
                    c.drawImage(ImageReader(io.BytesIO(symbols_map[sym['type']])), center_x - 20, target_y, width=40, height=40, mask='auto')
                    c.setFont("Helvetica-Bold", 8)
                    c.drawCentredString(center_x, target_y + 45, sym['label'])
...
with st.sidebar:
    ...
    charger = st.file_uploader("Charger (PNG)", type=['png', 'jpg'])
    choke = st.file_uploader("Choke (PNG)", type=['png', 'jpg'])
    sig_data = {"prep": st.text_input("Prep", "JE"), "chk1": st.text_input("SSE", "SSE"), "chk2": st.text_input("ASTE", "ASTE"), "app": st.text_input("DSTE", "DSTE")}
...
if uploaded_file:
    ...
    if st.button("Generate Final PDF"):
        s_map = {}
        if charger: s_map["CHARGER"] = charger.getvalue()
        if choke: s_map["CHOKE"] = choke.getvalue()
        pdf = process_multi_sheet_pdf(sheets, sig_data, s_map)
        st.download_button("Download PDF", pdf, "CTR_Final.pdf")